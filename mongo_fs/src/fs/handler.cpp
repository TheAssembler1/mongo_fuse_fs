...
#define POSTHANDLER_PRINT                                                               \
    std::cout << "done executing function: " << __FUNCTION__ << " handler" << std::endl \
              << "===================================" << std::endl
#define U                            \
    {                                \
        PREHANDLER_PRINT;            \
...
int Operations::mkdir(const char* path, mode_t mode) {
    ...
    mongo::FSMetadataCollectionEntry fs_metadata_collection_entry{
    FSHelper::get_base_name_of_path(path).c_str(), mode, mongo::MDFileType::DIR};
...
// NOTE: MODE 1:
// https://libfuse.github.io/doxygen/structfuse__operations.html#ae269583c4bfaf4d9a82e1d51a902cd5c
...
int Operations::readdir(const char* path, void* data, fuse_fill_dir_t filler, off_t offset, fuse_file_info* ffi, fuse_readdir_flags fdf) {
    ...
    std::vector<mongo::FSMetadataCollectionEntry> children =
    mongo::FSMetadataCollection::get_child_md_entries_of_parent_dir(parent_inode);
