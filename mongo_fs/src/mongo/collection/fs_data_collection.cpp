...
std::optional<FS_DATA_ID> FSDataCollection::create_entry(FSDataCollectionEntry& fs_data_collection_entry) {
  ...
  b_binary data{binary_sub_type::k_binary, static_cast<std::uint32_t>(BLOCK_SIZE), reinterpret_cast<const std::uint8_t*>(fs_data_collection_entry.buf)};
  
  std::cout << "creating entry for fs_data_collection" << std::endl;
  int fs_data_id = Manager::generate_id();
  file_collection.insert_one(make_document(
...
void FSDataCollection::update_entry(FSDataCollectionEntry& fs_data_collection_entry) {
  ...
  collection.update_one(
        make_document(kvp(FSDataCollectionEntry::FS_DATA_ID_KEY, fs_data_collection_entry.fs_data_id.value())),
...
FSDataCollectionEntry FSDataCollectionEntry::bson_to_entry(value bson_doc) {
  ...
  char* writable_bytes = new char[BLOCK_SIZE];
