...
std::optional<FS_DATA_ID> FSDataCollection::create_entry(
FSDataCollectionEntry& fs_data_collection_entry) {
    Connection conn;
    auto file_collection = GET_FS_DATA_COLLECTION(&conn);

    b_binary data{binary_sub_type::k_binary,
    static_cast<std::uint32_t>(fs::FSConfig::BLOCK_SIZE),
    ...
    make_document(kvp(FSDataCollectionEntry::FS_DATA_ID_KEY, fs_data_id),
...
void FSDataCollection::update_entry(FSDataCollectionEntry& fs_data_collection_entry) {
    ...
    collection.update_one(make_document(kvp(FSDataCollectionEntry::FS_DATA_ID_KEY,
                          fs_data_collection_entry.fs_data_id.value())),
...
FSDataCollectionEntry FSDataCollectionEntry::bson_to_entry(value bson_doc) {
    const char* bytes =
    (const char*)bson_doc[FSDataCollectionEntry::FS_BUF_KEY].get_binary().bytes;
    ...
    bson_doc[FSDataCollectionEntry::FS_DATA_ID_KEY].get_int32(), writable_bytes};
}
...
std::optional<FSDataCollectionEntry> FSDataCollection::read_entry(FS_DATA_ID fs_id) {
    ...
    auto bson_res = file_collection.find_one(
    make_document(kvp(FSDataCollectionEntry::FS_DATA_ID_KEY, fs_id)));
