...
int CollectionHelper::write_fuse_bufvec_to_mongo(const char* path,
fuse_bufvec& f_bvec,
off_t offset,
fuse_file_info& ffi) {
    assert(f_bvec.off == 0);
    ...
    for(; f_bvec.idx < f_bvec.count; f_bvec.idx++) {
        ...
        if(offset + f_buf.size > cur_file_size) {
            ...
            std::cout << "creating " << additional_blocks_needed
                      << " additional blocks" << std::endl;
        ...
        while(cur_fp - starting_fp < buf_size) {
            ...
            auto lookup_entry =
            FSLookupCollection::read_entry_with_inode_order(ffi.fh, block_order).value();
            std::cout << "writing block with fs data id: " << lookup_entry.fs_data_id
                      << std::endl;
            auto data_entry =
            FSDataCollection::read_entry(lookup_entry.fs_data_id).value();

            // NOTE: start at current position within block based on FP
            for(int pos_in_block = cur_fp % fs::FSConfig::BLOCK_SIZE;
                pos_in_block < fs::FSConfig::BLOCK_SIZE;) {
                ...
                data_entry.buf[pos_in_block] =
                ((const char*)f_buf.mem)[cur_fp - starting_fp];
...
// bvec...
void CollectionHelper::read_mongo_to_fuse_bufvec(const char* path,
fuse_bufvec** _f_bvec,
size_t size,
off_t offset,
fuse_file_info& ffi) {
    ...
    *_f_bvec =
    new fuse_bufvec[sizeof(struct fuse_bufvec) + 1 * sizeof(struct fuse_buf)]();
    auto f_bvec = *_f_bvec;
    ...
    while(cur_fp - starting_fp < buf_size) {
        ...
        auto lookup_entry =
        FSLookupCollection::read_entry_with_inode_order(ffi.fh, block_order).value();
        std::cout << "reading block with fs data id: " << lookup_entry.fs_data_id
                  << std::endl;
